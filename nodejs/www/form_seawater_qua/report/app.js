...
let provStation = (prov) => {
    axios.post("/sq-api/getstation", { prov, type: "user", usrid: urid }).then(r => {
        // console.log(r.data.data);
...
let callChart = () => {
    ...
    if (sta_n == "ทุกสถานีตรวจวัดค่า" && sq_pro !== "ทุกจังหวัด") {
        ...
        axios.post("/sq-api/getdatabysta", { sta: sta_n, type: "user", usrid: urid }).then((r) => {
            r.data.data.map(i => {
                sq_po43p.push({ cat: i.sq_date, dat: i.sq_po43p ? Number(i.sq_po43p) : null });
                sq_no3n.push({ cat: i.sq_date, dat: i.sq_no3n ? Number(i.sq_no3n) : null });
                sq_ph.push({ cat: i.sq_date, dat: i.sq_ph ? Number(i.sq_ph) : null });
...
let checkdata = async () => {
    await axios.post('/sq-api/getdata/user', { type: "ทุกจังหวัด", dat: "ทุกจังหวัด", usrid: urid }).then(r => {
        let d = r.data.data
        if (f_seawater_qua == 'false') {
            $("#noauth").modal("show")
        } else {
