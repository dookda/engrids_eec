...
let getMap = (x) => {
    ...
    x.map(i => {
        ...
        if (i.img) {
            marker
                .bindPopup(`<h6><b>คุณภาพอากาศ :</b> ${i.feeling}</h6>
                <h6><b>ที่ตั้ง :</b> ต.${i.tambon ? i.tambon : 'ไม่ระบุ'}  อ.${i.amphoe ? i.amphoe : 'ไม่ระบุ'} จ.${i.province ? i.province : 'ไม่ระบุ'}</h6><br><img class="border-10" src="${i.img}" width="100%">`)
                .addTo(map)
        } else {
            marker
                .bindPopup(`<h6><b>คุณภาพอากาศ :</b> ${i.feeling}</h6><h6><b>ที่ตั้ง :</b> ต.${i.tambon ? i.tambon : 'ไม่ระบุ'}  อ.${i.amphoe ? i.amphoe : 'ไม่ระบุ'} จ.${i.province ? i.province : 'ไม่ระบุ'}</h6>`)
                .addTo(map)
        }
    })
}
...
let confirmDelete = (datreport, dattime, id_date) => {
    ...
    if (dattime !== null) { $("#projTime").text(`เวลา ${dattime}`) }
